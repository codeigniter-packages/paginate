...
class Paginate {
    ...
    function fuzzy($str, $config) {
        $this->db->like($config['field_name'], $str, 'both');
    }
    ...
    function precise($str, $config) {
        ...
        if (strstr($str, ',')) {
            $this->db->where_in($config['field_name'], array_map('strval', preg_split('/,/', $str)));
        } elseif (strstr($str, '*')) {
            //如果是模糊查询
            if (substr($str, 0, 1) == '*' && substr($str, -1, 1) != '*') {
                $this->db->like($config['field_name'], preg_replace('/\*/', '', $str), 'before');
            } elseif (substr($str, 0, 1) != '*' && substr($str, -1, 1) == '*') {
                $this->db->like($config['field_name'], preg_replace('/\*/', '', $str), 'after');
            } else {
